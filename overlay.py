...
parser.add_argument('-lon', '--longitude', type=float, help='map center longitude, one per osm file (e.g. -3.1977 -.2326)', nargs='*')
parser.add_argument('-lat', '--latitude', type=float, help='map center latitude, one per osm file (e.g. 55.9486 51.5465)', nargs='*')
...
def addlayer(data, name, color):
  ...
  for i in range(len(args['filter'])):
    r = Rule()
    r.filter = Filter(args['filter'][i])
    line = LineSymbolizer(Color(color), args['stroke_width'][i])
...
for i in range(len(args['osmfile'])):
  ...
  # identify center
  center = m.layers[-1].envelope().center()
  ...
  if args['up'][i] == 'S': # south-up: new up pole is at center.x, center.y-90
    ...
    center.x = 180 + center.x
    center.y = 90 - center.y
    
  m.layers[-1].srs = '+proj=ob_tran +o_proj=latlong +lon_0=180 +o_lon_p=' + str(center.x) + ' +o_lat_p=' + str(center.y) + ' +to_meter=0.017453292519943295'
